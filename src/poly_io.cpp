...
namespace {
    ...
    std::tuple<float, float, Polygon> DefaultPolygonReader::CreatePointAndPolygonFromString(
        ...
        if (split_string.size() & 1){
            throw std::runtime_error("Missing point values");
        }
        ...
        if(polygon.x_vec_.size() != polygon.y_vec_.size()) {
            throw std::runtime_error("Incomplete values for polygon coordinates");
        }
